...
const chartData = {
    labels: [] as string[],
    datasets: [
        { label: 'CPU', data: [] as number[], borderColor: '#3b82f6', tension: 0.4, borderWidth: 2, pointRadius: 0 },
        { label: 'Memory', data: [] as number[], borderColor: '#6366f1', tension: 0.4, borderWidth: 2, pointRadius: 0 }
    ]
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const savedToken = localStorage.getItem('remote_helper_token');
    if (savedToken) {
...
window.authenticate = async function() {
    ...
    try {
        await invoke('authenticate', { token: authToken });
        // Success
        ...
        localStorage.setItem('remote_helper_token', authToken);
        window.refreshAll();
    } catch (e) {
        // Error
...
function updateChart(status: SystemStatus) {
    ...
    chartData.datasets[1].data.push((status.memory_usage / status.total_memory) * 100);
    chart.update();
}
