...
pub async fn start_service(state: &AppState, id: usize) -> Result<()> {
    let config = &state.config.service.get(id).context("Service ID not found")?;
    ...
    let child = cmd.spawn().context(format!("Failed to spawn {}", config.description))?;

    {
        let mut processes = state.service_processes.lock().unwrap();
...
pub async fn start_web_tunnel(state: &AppState) -> Result<()> {
    ...
    {
        let mut tunnel = state.web_tunnel_process.lock().unwrap();
        *tunnel = Some(child);
    }
